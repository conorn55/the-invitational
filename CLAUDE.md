...
- `.view-tab` buttons (absolutely positioned, both anchored to the top-right, so they add no header height) call `showView`, which toggles `hidden` on the leaderboard table, hint and `#schedule` table. `#schedule` reuses the `.leaderboard` table styles; `init()` loads `data/schedule.csv` (`Game,Date,Location,Complete`, dates `DD/MM/YYYY`, `Complete` is `Yes`/`No`), `parseScheduleCsv` sorts by date (all games shown, none dropped), and `renderSchedule` pads to `MIN_SCHEDULE_ROWS` with dash rows. A row with `Complete=Yes` gets the `complete` class, which strikes through its text via CSS rather than hiding the row.
