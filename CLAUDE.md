...
- `js/app.js` `init()` fetches `data/players.json` and `data/config.json` in parallel, adds derived rates (`withDerived`), sorts by `points` descending, then renders the header, header stats, and table. Rank is array position after sorting; it is never stored in the data.
...
- Player names pass through `escapeHtml`. Quotes inside names must be JSON-escaped (`"Steve \"The Joker\""`); invalid JSON makes the whole table render empty.
...
- **Mobile** (`max-width: 560px`): the Games column (3rd column) is hidden via `nth-child(3)`, so adding or reordering columns affects that rule.

## README drift

`README.md` still describes header stats as top-right tiles with three default entries; the current layout is a centred header with stats disabled.
